...
int main(int argc, char * argv[])
{
  ...
  RCLCPP_INFO(node->get_logger(), "Path computation: %.2f%% achieved", fraction * 100.0);

  // 3. Execute if feasible
  // We allow execution if > 90% of the path is found
  if (fraction > 0.9) {
    RCLCPP_INFO(node->get_logger(), "Executing Cartesian Move...");
    move_group.execute(trajectory);
  } else {
    RCLCPP_ERROR(node->get_logger(), "Path incomplete! Robot cannot reach target in a straight line.");
  }
